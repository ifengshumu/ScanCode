...
- (void)flashlight {
    if (!self.flashBtn) {
        ...
        [btn setFrame:CGRectMake((SCREEN_WIDTH-120)/3.0, CGRectGetMaxY(self.scanRect)+60, 60, 60)];
...
- (void)picButton {
    if (!self.picBtn) {
        ...
        btn.layer.cornerRadius = 30;
        btn.layer.masksToBounds = YES;
        [btn setFrame:CGRectMake((SCREEN_WIDTH-120)/3.0*2+60, CGRectGetMaxY(self.scanRect)+60, 60, 60)];
        [btn setImage:image forState:UIControlStateNormal];
